...
class FRANe:
    def __init__(self, iterations=100, metric='correlation',
                 min_edge_threshold=1.0, threshold_function='geomspace',
                 page_rank_decay=0.85,
                 page_rank_iterations=1000, page_rank_no_weights=False,
    ...
    def fit_page_rank(self, data, transpose=True):
        ...
        # hack for non zero min
        distances_copy[distances_copy == 0.0] = d_max + 1.0
        d_min = np.min(distances_copy, axis=None)
        thresholds = self.threshold_function(
            d_min, d_max, self.iterations, distances, sorted_indexes
        )
        ...
        if self.save_all_scores:
            self.feature_importances_ = [solution[-1]
